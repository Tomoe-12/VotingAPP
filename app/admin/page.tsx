...
export default function AdminPage() {
  ...
  const handleFullReset = async () => {
    try {
      ...
      alert("Success: All candidates' votes and voter tokens have been reset!");
    } catch (error) {
  ...
  const handleGenerateAlias = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/admin/token-alias", {
        ...
        body: JSON.stringify({
          password: adminToken,
          canonicalToken: aliasCanonical.trim(),
        }),
      });
    ...
    try {
      const uploadedUrls = await Promise.all(
        newImageFiles.map((file) =>
          uploadImageFile(file, "candidates", adminToken),
        ),
      );
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage candidates and election results
            </p>
          </div>
          {/* <Link href="/">
          ...
          <CardHeader>
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
              <div>
                ...
                <CardDescription>
                  Manage the voting process and results
                </CardDescription>
              </div>
          ...
          <CardContent>
            <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2">
              ...
              <Button
                ...
                //   if (
                //     confirm(
                //       "Are you sure? This will clear ALL votes and let everyone vote again!",
                //     )
                //   ) {
                //     handleFullReset();
                //   }
        ...
        <Card className="mb-8 border-border">
          <CardHeader>
            ...
            <CardDescription>
              Current voting statistics and leader board
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {candidates
                    ...
                    .map((candidate, index) => {
                      const totalKingVotes = candidates
                        .filter((c) => c.category === "king")
                        .reduce((sum, c) => sum + c.votes, 0);
                      const percentage =
                        totalKingVotes > 0
                          ? ((candidate.votes / totalKingVotes) * 100).toFixed(
                              1,
                            )
                          : "0.0";
                      return (
                  ...
                  {candidates.filter((c) => c.category === "king").length ===
                    0 && (
              ...
              <div>
                ...
                <div className="space-y-3">
                  {candidates
                    ...
                    .sort((a, b) => b.votes - a.votes)
                    .map((candidate, index) => {
                      const totalQueenVotes = candidates
                        .filter((c) => c.category === "queen")
                        .reduce((sum, c) => sum + c.votes, 0);
                      const percentage =
                        totalQueenVotes > 0
                          ? ((candidate.votes / totalQueenVotes) * 100).toFixed(
                              1,
                            )
                          : "0.0";
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <span
                                className={`text-lg font-bold ${index === 0 ? "text-secondary" : "text-muted-foreground"}`}
                              >
                                #{index + 1}
                              </span>
                              <span className="font-medium">
                  ...
                  {candidates.filter((c) => c.category === "queen").length ===
                    0 && (
                    <p className="text-muted-foreground text-sm">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  value={newCategory}
                  onValueChange={(value: "king" | "queen") =>
            ...
            <Card key={candidate.id} className="border-border">
              <CardContent className="p-4">
                <div className="flex items-center gap-4">
                  ...
                  <Button
                    onClick={() =>
                      handleRemoveClick(candidate.id, candidate.name)
                    }
                    variant="ghost"
                    size="icon"
